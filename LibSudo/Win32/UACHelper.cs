...
namespace LibSudo.Win32
{
    ...
    internal static class UACHelper
    {
        ...
        public static bool IsUserInAdminGroup()
        {
            ...
            DeepDarkWin32Fantasy.SafeTokenHandle hToken = null;
            DeepDarkWin32Fantasy.SafeTokenHandle hTokenToCheck = null;
            ...
            try
            {
                ...
                if (Environment.OSVersion.Version.Major >= 6)
                {
                    ...
                    cbSize = sizeof(Advapi32.TOKEN_ELEVATION_TYPE);
                    ...
                    if (!Advapi32.GetTokenInformation(hToken,
                        DeepDarkWin32Fantasy.TOKEN_INFORMATION_CLASS.TokenElevationType, pElevationType,
                        cbSize, out cbSize))
                    {
                        throw new Win32Exception();
                    }
                    ...
                    Advapi32.TOKEN_ELEVATION_TYPE elevType = (Advapi32.TOKEN_ELEVATION_TYPE)
                        Marshal.ReadInt32(pElevationType);

                    // If limited, get the linked elevated token for further check.
                    if (elevType == Advapi32.TOKEN_ELEVATION_TYPE.TokenElevationTypeLimited)
                    {
                        ...
                        if (!Advapi32.GetTokenInformation(hToken,
                            DeepDarkWin32Fantasy.TOKEN_INFORMATION_CLASS.TokenLinkedToken, pLinkedToken,
                            cbSize, out cbSize))
                        {
                            throw new Win32Exception();
                        }
                        ...
                        hTokenToCheck = new DeepDarkWin32Fantasy.SafeTokenHandle(hLinkedToken);
                    }
                }
                ...
                if (hTokenToCheck == null)
                {
                    if (!Advapi32.DuplicateToken(hToken,
                        Advapi32.SECURITY_IMPERSONATION_LEVEL.SecurityIdentification,
                        out hTokenToCheck))
                    {
                        throw new Win32Exception();
                    }
        ...
        public static bool IsProcessElevated()
        {
            ...
            try
            {
                ...
                cbTokenElevation = Marshal.SizeOf(typeof(DeepDarkWin32Fantasy.TOKEN_ELEVATION));
                ...
                if (!Advapi32.GetTokenInformation(hToken,
                    DeepDarkWin32Fantasy.TOKEN_INFORMATION_CLASS.TokenElevation, pTokenElevation,
                ...
                DeepDarkWin32Fantasy.TOKEN_ELEVATION elevation = (DeepDarkWin32Fantasy.TOKEN_ELEVATION)Marshal.PtrToStructure(
                    pTokenElevation, typeof(DeepDarkWin32Fantasy.TOKEN_ELEVATION));
        ...
        public static int GetProcessIntegrityLevel()
        {
            ...
            try
            {
                ...
                if (!Advapi32.GetTokenInformation(hToken,
                    DeepDarkWin32Fantasy.TOKEN_INFORMATION_CLASS.TokenIntegrityLevel, IntPtr.Zero, 0,
                    out cbTokenIL))
                {
                    int error = Marshal.GetLastWin32Error();
                    if (error != DeepDarkWin32Fantasy.ERROR_INSUFFICIENT_BUFFER)
                    {
                ...
                if (!Advapi32.GetTokenInformation(hToken,
                    DeepDarkWin32Fantasy.TOKEN_INFORMATION_CLASS.TokenIntegrityLevel, pTokenIL, cbTokenIL,
                    out cbTokenIL))
                {
                    throw new Win32Exception();
                }
                ...
                DeepDarkWin32Fantasy.TOKEN_MANDATORY_LABEL tokenIL = (DeepDarkWin32Fantasy.TOKEN_MANDATORY_LABEL)
                    Marshal.PtrToStructure(pTokenIL, typeof(DeepDarkWin32Fantasy.TOKEN_MANDATORY_LABEL));

                // Integrity Level SIDs are in the form of S-1-16-0xXXXX. (e.g. 
                // S-1-16-0x1000 stands for low integrity level SID). There is one 
                // and only one subauthority.
                IntPtr pIL = Advapi32.GetSidSubAuthority(tokenIL.Label.Sid, 0);
